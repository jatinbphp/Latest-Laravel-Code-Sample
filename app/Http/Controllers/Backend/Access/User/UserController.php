...
class UserController extends Controller
{
    ...
    public function alluser(ManageUserRequest $request)
    {
		
        return view('backend.access.alluser');
    }
    ...
    public function get(ManageUserRequest $request) {
        ...
        return Datatables::of($this->users->getForDataTableNotAdministrator($request->get('status'), $request->get('trashed')))
    ...
    {		
		...
		if(!empty($input['password_field'])){
			if($input['password_field']!=$input['password_field_confirm']){			
				throw new GeneralException(trans('Password and Confirm password must be same!!'));
			}
		}
    ...
    public function destroy(User $user, ManageUserRequest $request)
    {
        ...
        return redirect()->back()->withFlashFail(trans('alerts.backend.users.deleted'));
    }
