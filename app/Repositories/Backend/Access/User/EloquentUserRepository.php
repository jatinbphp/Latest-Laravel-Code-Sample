...
class EloquentUserRepository implements UserRepositoryContract
{
    ...
    public function getForDataTableNotAdministrator($status = 1, $trashed = false)
    {
        ...
        if ($trashed == "true") {
            return User::onlyTrashed()
                ->select(['id', 'name', 'email', 'status','city','state','zip','confirmed', 'created_at', 'updated_at', 'deleted_at','address','subscription'])
                ->groupBy('id')
                ->get();
        }

        return  User::select(['users.id', 'name', 'email', 'status','city','state','zip', 'confirmed', 'created_at', 'updated_at', 'deleted_at','address','subscription'])
    ...
    public function update(User $user, $input, $roles)
    {
		...
		DB::transaction(function() use ($user, $input, $roles) {
			if ($user->update($input)) {
				...
				if(!empty(trim($input['password_field']))){					
					$user->password = bcrypt($input['password_field']);	
				}
